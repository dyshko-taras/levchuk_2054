...
- [ ] Add required imports to `main.dart`:
  ```dart
  import 'package:flutter/services.dart';
  ```
- [ ] Create `DisplayMode` enum in `main.dart`:
  - [ ] Add `immersive` mode
  - [ ] Add `normal` mode
- [ ] Implement `setDisplayMode()` function:
  - [ ] Use `SystemChrome.setEnabledSystemUIMode(SystemUiMode.immersiveSticky)`
...
- [ ] In `main()` function (before `runApp()`):
  ...
  - [ ] Enable immersive mode: `await setDisplayMode(DisplayMode.immersive)`
  - [ ] Lock orientation to landscape:
    ...
    ```
- [ ] Add `WidgetsBindingObserver` mixin to main app state
- [ ] Implement `didChangeAppLifecycleState()`:
  - [ ] Pause audio on `AppLifecycleState.paused`/`inactive`
  - [ ] Resume audio and re-enable immersive mode on `AppLifecycleState.resumed`
- [ ] Test immersive mode on device (simulator may not show correctly)
- [ ] Verify system bars auto-hide after swiping from edge

**Checkpoint**: App runs in fullscreen landscape mode with hidden system bars.
...
- [ ] Implement ButtonWidthStep enum with configurable step values (e.g., 200/300/400px)
- [ ] Add automatic width calculation using TextPainter with your button's exact TextStyle
- [ ] Include BuildContext parameter in getSteppedWidth() for textScaler access
- [ ] Use exact TextStyle matching your button's rendered text (font family, size, weight)
- [ ] Test automatic width sizing with various label lengths
- [ ] Verify buttons with similar labels get consistent widths (no overflow)
- [ ] Test manual width override with minWidth parameter
- [ ] Test with accessibility text scaling enabled
...
- [ ] Add fade-in overlay
- [ ] Wrap scrollable content in FadingScrollView
...
- [ ] Create `lib/widgets/fading_scroll_view.dart` (scrollable content with edge fading)
- [ ] Create `lib/widgets/hud_stat_indicator.dart` (game HUD)
- [ ] Create `lib/widgets/widgets.dart` (export all widgets)
...
- [ ] Wrap ListView in FadingScrollView for edge fading
