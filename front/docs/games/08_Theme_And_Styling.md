...
class AppColors {
  ...
  // Button role-based colors (for GameButtonStyle enum)
  static const Color buttonPrimary = neonCyan;      // Main actions
  static const Color buttonSecondary = neonAmber;   // Cancel/back actions
  static const Color buttonDestructive = error;     // Warning/delete actions
  static const Color buttonNeutral = neonPurple;    // Utility/debug actions
...
- [ ] Define button role-based colors (primary, secondary, destructive, neutral)
...
### GameButton Style - Role-Based Approach

**Modern Approach**: Use semantic role-based styling instead of manual color specification.

```dart
// Define role-based button styles
enum GameButtonStyle {
  primary,      // Main actions (PLAY, CONFIRM, OK)
...
// Implementation maps roles to theme colors
Color _getBorderColor() {
  switch (style) {
    case GameButtonStyle.primary:
      return AppColors.buttonPrimary;      // e.g., Gold
    case GameButtonStyle.secondary:
      return AppColors.buttonSecondary;    // e.g., Chrome/Silver
    case GameButtonStyle.destructive:
      return AppColors.buttonDestructive;  // e.g., Red
    case GameButtonStyle.neutral:
      return AppColors.buttonNeutral;      // e.g., Steel/Gray
  }
}
```

**Benefits**:
- **Semantic clarity**: Button role is clear from code
- **Theme-agnostic**: Colors change with theme without code updates
- **Consistency**: All buttons of same role look identical
- **Maintainable**: Change theme colors in one place
- **Reusable**: Template-friendly across projects

**Legacy Manual Styling** (avoid for new code):
```dart
Container(
  decoration: BoxDecoration(
    ...
    borderRadius: BorderRadius.circular(12),
    border: Border.all(
      color: AppColors.neonCyan,  // Hardcoded - not role-based
      width: 2,
    ),
    boxShadow: [
      BoxShadow(
