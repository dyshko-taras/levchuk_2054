...
The game requires specific platform configurations for an optimal gaming experience. These are set up in `main.dart` before the app starts:

### Display Mode (Immersive Fullscreen)

Games should run in **immersive mode** with system UI bars hidden for maximum screen real estate:

```dart
enum DisplayMode {
  immersive,  // Hide status bar and navigation bar
  normal,     // Show system UI
}

Future<void> setDisplayMode(DisplayMode mode) async {
  if (mode == DisplayMode.immersive) {
    await SystemChrome.setEnabledSystemUIMode(
      SystemUiMode.immersiveSticky,  // Auto-hides bars when they reappear
    );
...
**Key Points**:
- Use `SystemUiMode.immersiveSticky` for automatic re-hiding of system bars
- Enable immersive mode in `main()` before `runApp()`
- Re-enable when app resumes from background in `didChangeAppLifecycleState()`
- Keep bars transparent for rare re-appearances

### Orientation Lock

Lock the game to **landscape orientation** for consistent gameplay:
...
Monitor app lifecycle to restore immersive mode and manage audio:

```dart
class _PuzzleboundAppState extends State<PuzzleboundApp>
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    switch (state) {
      ...
      case AppLifecycleState.inactive:
        audioService.onAppPaused();  // Pause music
      case AppLifecycleState.resumed:
        audioService.onAppResumed();  // Resume music
        setDisplayMode(DisplayMode.immersive);  // Restore fullscreen
      // ...
    }
  }
}
...
- **Enable immersive mode early** - Set up fullscreen with hidden system bars from the start
- **Test on real devices** - Immersive mode and orientation locks may not work correctly in simulators
...
- **Handle lifecycle** - Always pause audio/game and restore immersive mode when app resumes
...
1. **System bars reappearing** - Re-enable immersive mode in `didChangeAppLifecycleState()` when app resumes
2. **Flame black background covering Flutter** - See 11_Flame_Nuances.md
3. **Audio playing in background** - Implement proper lifecycle handling
4. **Tight coupling** - Use service locator and callbacks
5. **Inconsistent theming** - Define colors and styles centrally
6. **No difficulty scaling** - Implement GameLoopManager pattern
7. **Poor state management** - Separate game state from UI state
8. **Memory leaks** - Dispose controllers and listeners properly
