...
## 12) Background Work & Scheduled Notifications

<CRITICAL_RULE>
### 12.1 Use WorkManager, NOT Exact Alarms

**NEVER use exact alarms** (`SCHEDULE_EXACT_ALARM`, `USE_EXACT_ALARM`) for scheduled notifications. These permissions:
- Require explanation to Google Play review team
- Risk app rejection
- Cause UX issues on Android 14+ (permission dialogs)

**ALWAYS use WorkManager** for scheduled notifications. It:
- Requires NO special permissions
- Passes Google Play review automatically
- Works reliably in background
- Is energy-efficient

**Acceptable timing variance:** ±5-15 minutes is normal for WorkManager and acceptable for most use cases (match reminders, weekly stats, etc.)
</CRITICAL_RULE>
...
# pubspec.yaml
dependencies:
  flutter_local_notifications: ^19.5.0  # For showing notifications
  workmanager: ^0.9.0                   # For scheduling background tasks
```

### 12.3 AndroidManifest Permissions
...
<!-- NEVER ADD THESE - will cause Google Play review issues -->
...
See detailed guide: [flutter_workmanager_notifications.md](../tech/flutter_workmanager_notifications.md)

**Quick reference:**

```dart
// 1. Callback dispatcher (must be top-level function)
...
### 12.5 When Exact Alarms ARE Required

Only if app's PRIMARY function is:
- Alarm clock
- Timer
- Medication reminders (critical timing)

For sports apps, news apps, social apps: **always use WorkManager**.
</CODE_PATTERN>
